...
def read_charge(file,typemap,atom_num):
    total = []
    with open(file) as f: 
        lines = f.readlines()[:]
        idx = 0
        for line in lines:
            idx += 1
            if "Atom  Element  Kind" in line:
                break
        for k in typemap:
            for i in range(atom_num):
                sp = lines[idx+i].split()
                if sp[1] == k:
                   total.append(float(sp[5]))        
    return total
...
for i in all_dir:
    ...
    save_path1 = os.path.join(data_path,  "dp_dataset_charge",i)
    ...
    charge_path= os.path.join(save_path2,  "atom_dos.npy")
    fake_path  = os.path.join(save_path2,  "dos.npy")
    ...
    for j in choose_frames:
        ...
        cp2k_chg  = os.path.join(cp2k_path,"m062x/M062X-hirshfeld-1.hirshfeld")
        ...
        total_charge.append(read_charge(cp2k_chg,typemap,atom_num))
    ...
    np.save(fake_path, total_energy_np*au2eV)

    total_chg_np = np.asarray(total_charge,dtype=float).reshape(len(choose_frames),atom_num)
    np.save(charge_path, total_chg_np)
